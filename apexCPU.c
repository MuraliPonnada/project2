...
void initCPU(cpu cpu) {
	...
	for(int i=0;i<16;i++) {
		...
		cpu->preg[i].value=cpu->reg[i];
		cpu->preg[i].v=cpu->regValid[i];
		cpu->preg[i].p=-1;
		cpu->preg[i].z=-1;
	}
...
void printState(cpu cpu) {
	...
	for (int s=0;s<=writeback;s++) {
		// printf("line 117  %d cpu->stage[s].status=%d: \n",s,cpu->stage[s].status);
...
void cycleCPU(cpu cpu) {
	...
	for(enum stage_enum fu=fu_alu3; fu<=fu_br3 && cpu->stage[writeback].status==stage_noAction; fu+=3) {
		if (cpu->stage[fu].status==stage_actionComplete || cpu->stage[fu].status==stage_ready) {
			...
			if(cpu->stage[fu].pc==cpu->rob[cpu->headtail[0]].pc){
				...
				cpu->headtail[0]=cpu->headtail[0]+1;
			}
			// printf("line 179 -> head=%d tail=%d\n",cpu->headtail[0],cpu->headtail[1]);
			// cpu->stage[writeback]=cpu->stage[fu];
			// cpu->stage[writeback].status=stage_ready;
			// cpu->stage[fu].status=stage_noAction; // Available for new
		}
	...
	if (cpu->stage[writeback].status==stage_noAction) {
		cpu->stage[writeback].status=stage_squashed; // Nothing is ready to writeback
	...
	if(cpu->stage[issue].status!=stage_stalled){
		if(cpu->stage[dispatch].status==stage_stalled){
			cpu->stage[issue].status=stage_squashed;
			cpu->stage[issue].instruction=0;
			cpu->stage[issue].opcode=0;
		} else {
			cpu->stage[issue]=cpu->stage[dispatch];
			if (cpu->stage[dispatch].status!=stage_stalled) {
				if(cpu->stage[decode].status==stage_stalled ) {
					...
					// printf("line 202 %d %d\n",cpu->stage[dispatch].status,cpu->stage[decode].status);
	...
	for(enum fu_enum fu=dispatch; fu<=br_fu+2; fu++) {
	...
	// printf("line 314 status of fetch=%d decode=%d dispatch=%d\n",cpu->stage[fetch].status,cpu->stage[decode].status,cpu->stage[dispatch].status);
}
...
void cycle_fetch(cpu cpu) {
	...
	for(int s=1;s<=writeback;s++) if (cpu->stage[s].status==stage_stalled) return;
	if (cpu->halt_fetch) {
...
void cycle_decode(cpu cpu) {
	...
	// If this was stalled last time... try again. It might be ready now
...
void cycle_dispatch(cpu cpu) {
	if (cpu->stage[issue].status!=stage_actionComplete) return;
	// Move decoded instruction onto fu
	enum fu_enum fu=cpu->stage[issue].fu;
	if (cpu->stage[fu].status==stage_noAction ||
		...
		if (cpu->stage[issue].op1Valid==0) {
			reportStage(cpu,issue,"Waiting for first operand");
			cpu->stage[issue].status=stage_stalled;
			return;
		}
		if (cpu->stage[issue].op2Valid==0) {
			reportStage(cpu,issue,"Waiting for second operand");
			cpu->stage[issue].status=stage_stalled;
			return;
		}
		...
		reportStage(cpu,issue," issued to fu %d",fu);
		cpu->stage[issue].status=stage_actionComplete;
	} else {
		// fu is not available, issue needs to stall
		reportStage(cpu,issue,"Required FU busy, status=%s",
