...
void mul_execute(cpu cpu) {
	...
	reportStage(cpu,fu_alu1,"res=%d*%d",cpu->stage[fu_mul1].op1,cpu->stage[fu_mul1].op2);
...
void load_execute(cpu cpu) {
	cpu->stage[fu_ld1].effectiveAddr =
		cpu->stage[fu_ld1].op1 + cpu->stage[fu_ld1].op2;
...
void fwd_dispatch(cpu cpu){
	int ht = cpu->headtail[1];
	cpu->rob[ht].a=1;
	cpu->rob[ht].areg=cpu->stage[dispatch].dr;
	cpu->rob[ht].preg=cpu->stage[dispatch].dr;
	cpu->rob[ht].pc=cpu->stage[dispatch].pc;
	cpu->headtail[1]=ht+1;
	return;
}
